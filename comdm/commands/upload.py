from helpers.filereader import ExcelReader
from helpers.filewriter import MongoWriter

class Upload:
    """Databases upload functionalities"""

    def __init__(self, source_path, source_name, desti, sheetname):
        self.source = source_path + '\\' + self._make_valid_xl_file(source_name)
        self.desti = desti
        self.xl_reader = ExcelReader(self.source, sheetname)
        self.mongo_writer = MongoWriter(self.desti)
    ...
    def validate(self):
        ...
        if not self.xl_reader.file_exists():
            ...
            return False, err
        if not self.mongo_writer.collection_exists():
            # Checks whether mongodb collection given exists
            err = "Error: %s collection does NOT exist!" % self.desti
            return False, err
        msg = "Info: Both %s file and %s collection exist" % (self.source, self.desti)
        return True, msg
    ...
    def read_xl_upload_mongo(self):
        ...
        self.mongo_writer.set_data(self.xl_reader.df
        self.mongo_writer.cleanup_existing_data()
        self.mongo_writer.write(self.xl_reader.rows)
